 ...
 * Click 1 = top-left, click 2 = bottom-right → copies full sprite rect.
 * Paste that line back into chat (with prop name).
 */
      ...
      <div className="pointer-events-none absolute bottom-2 left-2 right-2 z-[10081] flex flex-wrap items-end justify-between gap-2">
        <div className="rounded bg-black/85 px-3 py-2 font-mono text-xs text-amber-100">
          <div className="text-[10px] uppercase tracking-wide text-amber-400/90">
            Place mode · footprint = position only (size is fixed) · P off
          </div>
          <div className="mt-1 text-sm">
            {!cornerA &&
              (cursor
                ? `Corner A (top-left): { x: ${cursor.x}, y: ${cursor.y} }`
                : 'Click top-left of where the prop should sit…')}
            {cornerA &&
              (draft
          ...
          {copied && (
            <div className="mt-1 text-lime-300">Copied: {copied}</div>
          )}
        </div>
        {rects.length > 0 && (
          <button
